...
import { getFirestore, collection,getDocs, doc } from 'https://www.gstatic.com/firebasejs/9.20.0/firebase-firestore.js'
...
const x = ref(db, "Company");

const data = getFirestore(app);

onValue(x , async (snapshot) => {
    if(snapshot.exists())
    {
        var d = Object.values(snapshot.val());
        for (let i = 0; i < d.length; i++) {
            const colref = collection(data,d[i]);
            ...
            querySnapshot.forEach((doc) => {
                ...
                const data1 = document.createTextNode(d[i]);
